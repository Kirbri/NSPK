...
public class UniversalQRPage {
    ...
    public UniversalQRPage checkHowToAcceptPaymentTapText() {
        ...
        howStepsText.should(text("Клиент считывает Универсальный QR: либо камерой смартфона, либо камерой платежного приложения"));
        stepsThemeWhite.get(1).click();
        howStepsText.should(text("Клиент выбирает удобный для себя способ оплаты из числа тех, которые поддерживаются продавцом, и платежное приложение"));
        stepsThemeWhite.get(2).click();
        howStepsText.should(text("Клиент подтверждает оплату. Денежные средства поступают на счет продавца в соответствии с договором между ним и тем платежным сервисом, с использованием которого выполнен платеж"));
        return this;
    }
